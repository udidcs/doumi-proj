...
function countBytes2(editor, containerSelector, limit) {
    //toast api일땐 getMarkdown 그냥 input일 경우 value
    const content = editor.getMarkdown ? editor.getMarkdown() : editor.value;
    //문자열을 바이트로 변환
    const byteCount =countUtf8Bytes2(content);
    // 글자 수를 표시할 요소에 업데이트
    $(containerSelector).text(byteCount + "/"+limit);
    ...
    if (byteCount > limit) {
        alert('허용된 글자수가 초과되었습니다.')
        const truncatedContent = cutByLen2(content, limit);
        editor.setMarkdown ? editor.setMarkdown(truncatedContent,false) : editor.value = truncatedContent; // 넘어간 부분을 잘라냄
    }
}

// 문자열을 주어진 바이트 수에 맞게 자르는 함수
function cutByLen2(str, maxByte) {
    for (let b = i = 0; c = str.charCodeAt(i);) {
        //줄 바꿈(/n)일 경우 byte 2, 나머지 1
        b+=c==10?2:1;
        if (b > maxByte) break;
...
function countBytes3(editor, containerSelector, limit) {
    //toast api일땐 getMarkdown 그냥 input일 경우 value
    const content = editor.getMarkdown ? editor.getMarkdown() : editor.value;
    //문자열을 바이트로 변환
    const byteCount =countUtf8Bytes3(content);
    // 글자 수를 표시할 요소에 업데이트
    $(containerSelector).text(byteCount + "/"+limit);
    ...
    if (byteCount > limit) {
        alert('허용된 글자수가 초과되었습니다.')
        const truncatedContent = cutByLen3(content, limit);
        editor.setMarkdown ? editor.setMarkdown(truncatedContent,false) : editor.value = truncatedContent; // 넘어간 부분을 잘라냄
    }
}

// 문자열을 주어진 바이트 수에 맞게 자르는 함수
function cutByLen3(str, maxByte) {
    for (let b = i = 0; c = str.charCodeAt(i);) {
        //줄 바꿈(/n)일 경우 byte 2, 나머지 1
        b+=c==10?2:1;
        if (b > maxByte) break;
